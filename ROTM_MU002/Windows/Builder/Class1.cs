    ...
    public class ContentBuilder : IDisposable
    {
        ...
        class ImporterGroup
        {
            ...
            public ImporterDef FindByExtension(string extension)
            {
                ...
                {
                    if (Importers[i].SupportedExtension.Equals(extension))
                        return Importers[i];
                }
                return null;
            }
        ...
        public ContentBuilder(string outs)
        {
            ...
            CreateTempDirectory();
            CreateBuildProject();
            Importers = new ImporterGroup();
            Importers.Add(new ImporterDef(".fx", "EffectImporter", "EffectProcessor"));
            Importers.Add(new ImporterDef(".fbx", "FbxImporter", "SkinnedModelProcessor"));
        }
        ...
        void CreateBuildProject()
        {
            ...
            buildProject.SetProperty("OutputPath", this.OutputDirectory);
            ...
            foreach (string pipelineAssembly in pipelineAssemblies)
            {
                ...
                if (pipelineAssembly.EndsWith(".dll"))
                {
                    ...
                    File.Copy(Path.Combine(workingDir, pipelineAssembly), Path.Combine(buildDirectory, assemblyReferencePath));
                }
                buildProject.AddItem("Reference", assemblyReferencePath);
            }
        ...
        {
            var importer = Importers.FindByExtension(Path.GetExtension(filepath).ToLower());
